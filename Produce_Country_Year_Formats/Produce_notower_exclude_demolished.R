...
sc$out <- ifelse(sc$status %in% c("Proposed","Vision","Never Completed","On Hold", "Demolished"), 1, 0) #Here lies the only difference from the main data
...
#####   Recode Yugoslavia- and USSR-countries, Czech Republic, and Hong Kong (Follow V-Dem's country-definition)  #####
...
#Save
df_demolished <- df
save(df_demolished,file="countryyear_ExclDemolished.rdata")
