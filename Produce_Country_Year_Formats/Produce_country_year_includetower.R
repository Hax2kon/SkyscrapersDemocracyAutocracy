...
load("buildings.rdata") #Make sure this is the path to were you saved the output from "Rep_Produce_Completed_Buildinglevel.R"

#####   \\    --  Prepare SkyScraperData   --    //  #####

#Remove everything that is never realized
...
#Extract Subset
df <- df[which(df$year>=1900 & df$year<=2016),c(ids,skyscrapers,democracy,economy,conflict,international,land,mediators)]
df <- df[which(is.na(df$country_id)==FALSE),]
...
##  Other ##
df$usa <- ifelse(df$country=="United States",1,0)
df$china <- ifelse(df$country=="China",1,0)
df$hongkong <- ifelse(df$country=="Hong Kong",1,0)
...
#Save
df_incltower <- df
save(df_incltower,file="country_year_Include_Towers.rdata")
